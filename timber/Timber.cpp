...
//Sprite* branches = (Sprite*)malloc(sizeof(Sprite) * NUM_BRANCHES);
//Sprite* branches = new Sprite[NUM_BRANCHES];
...
int main()
{
	...
	//Sprite branches[NUM_BRANCHES] = {textureBranch, textureBranch, textureBranch, textureBranch, textureBranch, textureBranch};

	// Set the texture for each branch sprite
	for(int i = 0; i < NUM_BRANCHES; i++)
	{
		//Sprite::Sprite branches[i]({textureBranch});
	...
	while (window.isOpen())
	{
		...
		*/
		...
		if(acceptInput)
		{
			...
			{
				// Make sure the player in on the right 
		...
		if(!paused)
		{
				...
				// Move the bee
			{
				...
				{
					// Set it up to be the whole new bee next frame
					beeActive = false;
				}
			}
			...
			else
			{
				...
				{
					// Set it up ready to be a whole new cloud new frame
					cloud1Active = false;
				}
			}
