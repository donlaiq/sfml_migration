...
void Factory::loadLevel(vector<GameObject>& gameObjects, VertexArray& canvas, InputDispatcher& inputDispatcher)
{
	...
	for(int i = 0; i < 8; ++i)
	{
		...
		shared_ptr<PlatformGraphics> platformGraphics = make_shared<PlatformGraphics>();
		platformGraphics->assemble(canvas, platformUpdate, IntRect({PLATFORM_TEX_LEFT, PLATFORM_TEX_TOP}, {PLATFORM_TEX_WIDTH, PLATFORM_TEX_HEIGHT}));
	...
	for(int h = -areaToCover / 2; h < areaToCover / 2; h += rainCoveragePerObject)
	{
		...
		{
			GameObject rain;
			shared_ptr<RainGraphics> rainGraphics = make_shared<RainGraphics>(playerUpdate->getPositionPointer(), h, v, rainCoveragePerObject);
			rainGraphics->assemble(canvas, nullptr, IntRect({RAIN_TEX_LEFT, RAIN_TEX_TOP}, {RAIN_TEX_WIDTH, RAIN_TEX_HEIGHT}));
	...
	const float width = float(VideoMode::getDesktopMode().size.x);
	const float height = float(VideoMode::getDesktopMode().size.y);
	...
	shared_ptr<CameraGraphics> cameraGraphics = make_shared<CameraGraphics>(m_Window, m_Texture, Vector2f(CAM_VIEW_WIDTH, CAM_VIEW_WIDTH / ratio), FloatRect({CAM_SCREEN_RATIO_LEFT, CAM_SCREEN_RATIO_TOP}, {CAM_SCREEN_RATIO_WIDTH, CAM_SCREEN_RATIO_HEIGHT}));
	...
	shared_ptr<CameraGraphics> mapCameraGraphics = make_shared<CameraGraphics>(m_Window, m_Texture, Vector2f(MAP_CAM_VIEW_WIDTH, MAP_CAM_VIEW_HEIGHT / ratio), FloatRect({MAP_CAM_SCREEN_RATIO_LEFT, MAP_CAM_SCREEN_RATIO_TOP}, {MAP_CAM_SCREEN_RATIO_WIDTH, MAP_CAM_SCREEN_RATIO_HEIGHT}));

	mapCameraGraphics->assemble(canvas, mapCameraUpdate, IntRect({MAP_CAM_TEX_LEFT, MAP_CAM_TEX_TOP}, {MAP_CAM_TEX_WIDTH, MAP_CAM_TEX_HEIGHT}));
	...
	menuGraphics->assemble(canvas, menuUpdate, IntRect({TOP_MENU_TEX_LEFT, TOP_MENU_TEX_TOP}, {TOP_MENU_TEX_WIDTH, TOP_MENU_TEX_HEIGHT}));
